 ...
 * 
 * 	An easy to use PHP Library you can use to make requestes to the Twitter 1.1
 *  API.
 * 
 *  @author  Jean Paul - PHPClan.com
 *  @version  0.3.1
 */
class EasyTwitter
{	
	...
	public function access_token_secret($oauth_token_secret)
	{
		$this->oauth_token_secret($oauth_token_secret);
	}

	public function get($base_url)
	{
		$method = $this->method ="GET";
		
		//if URL contains params let's extract them
		...
		//Let's make sure the url doesn't contain the query
	...
	public function media($media_content, $binary=false)
	{
		if(!$binary){

			$media = file_get_contents($media_content);
		} else {
			$media = $media_content;
		}
		...
		$response = $this->post($base_url, true);
		return $response;
	}
